...
def handle_armor_enter(obj: types.CodeType):
    ...
    while i < len(raw_code):
        ...
        if op in absolute_jumps:
            ...
            while raw_code[
                i - 2] == EXTENDED_ARG:  # Remove the preceding extended arguments, we add our custom ones later on
...
def find_modules(directory):
    for filename in os.listdir(directory):
        if os.path.isdir(filename):
            ...
            if not os.path.exists(os.path.join("dump", os.path.join(directory, os.path.splitext(filename)[
                                                                                   0] + ".pyc"))) and b"__pyarmor__" in open(
                    path, "rb").read() and os.path.abspath(path) != os.path.abspath(__file__):
...
def log(event, arg):
    if event == "marshal.loads" and not globals()["triggered"] and b"frozen" in arg[
        0]:  # We have to use globals() because we're inside an audit hook
        try:
